...
*
*/
const week_compose = (y:number, m:number, d:number) => {
  let _arr = [...Array(7)];
  let _dn = dayNumber(d, m, y);
  let cnt = 0;
  for (let i = -(_dn); i < -(_dn)+7; i++) {
    let tmp_date = new Date(y, m-1, d+i);
    _arr[cnt] = {
      date : `${tmp_date.getFullYear()}_${pad_integer_with_zeros(tmp_date.getMonth()+1,2)}_${pad_integer_with_zeros(tmp_date.getDate(),2)}`
    }
    cnt++;
  }
  return _arr;
